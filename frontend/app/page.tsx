...
export default function LandingPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', maxWidth: 1100 }}>
          ...
          <div className="animate-fade-up delay-3" style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/login" className="btn btn-solid" style={{ fontSize: '1.1rem', padding: '14px 32px' }}>
              Begin Residency
            </Link>
            <a href="#modules" className="btn btn-outline" style={{ fontSize: '1.1rem', padding: '14px 32px' }}>
              Explore Modules
            </a>
          </div>
          ...
          <div className="animate-fade-up delay-3" style={{
            display: 'flex', gap: '6vw', justifyContent: 'center', marginTop: 100,
            ...
            {[
              ...
              <div key={s.label} style={{ textAlign: 'center', minWidth: 140 }}>
                <div style={{ fontSize: '3rem', fontWeight: 500, letterSpacing: '-0.05em', lineHeight: 1, color: s.color }}>{s.val}</div>
                <div style={{ color: 'var(--text-secondary)', marginTop: 12, fontSize: '0.9rem', fontWeight: 500 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── MODULES ──────────────────────── */}
      <section id="modules" style={{ padding: '100px 24px', maxWidth: 1400, margin: '0 auto', width: '100%' }}>
        <div style={{ marginBottom: 64, textAlign: 'center' }}>
          <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3.5rem)', marginBottom: 16, lineHeight: 1.2, fontWeight: 700 }}>
            Four Modules. Real Crises.
          </h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24 }}>
          {modules.map((m, i) => (
            ...
            >
              ...
              <h3 style={{ fontSize: '1.5rem', marginBottom: 12, fontWeight: 500 }}>{m.title}</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem', lineHeight: 1.5, flexGrow: 1, marginBottom: 32 }}>
                {m.desc}
              </p>
                  ...
                  {Array.from({ length: 5 }).map((_, j) => (
                    <div
                      ...
                      style={{
                        ...
                        background: j < m.difficulty ? 'var(--google-blue)' : 'var(--border)',
                      }}
                    />
                  ))}
                  <span style={{ color: 'var(--text-muted)', marginLeft: 8, fontSize: '0.8rem', fontWeight: 500 }}>
                    DIFFICULTY
                  </span>
                </div>
                <span style={{ fontSize: '1.2rem', color: 'var(--google-blue)' }}>→</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', marginBottom: 24, lineHeight: 1.2, fontWeight: 700 }}>
            Ready for Day 1?
          </h2>
          <p style={{ color: 'var(--text-secondary)', marginBottom: 48, fontSize: '1.1rem', maxWidth: 540, margin: '0 auto 48px' }}>
            Free to use. No credit card required. Authenticate with your developer account and start debugging.
          </p>
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={loginWithGitHub} className="btn btn-outline" style={{ minWidth: 220, padding: '12px 24px' }}>
            ...
            <button onClick={loginWithGoogle} className="btn btn-solid" style={{ minWidth: 220, padding: '12px 24px' }}>
      ...
      <footer style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: '1.2rem', fontWeight: 600, color: 'var(--google-blue)' }}>
            CodeResidency
          </span>
        </div>
